define(["../main", "doh", "require", "../_base/lang","../cache", "../_base/url"], function(dojo, doh, require, lang) {
	doh.register("tests.cache", [{
			name: "dojo.cache",
			runTest: function(t){
				var expected = "<h1>Hello World</h1>";

				t.is(expected, lang.trim(dojo.cache("dojo.tests.cache", "regular.html")));
				t.is(expected, lang.trim(dojo.cache("dojo.tests.cache", "sanitized.html", {sanitize: true})));

				//Test object variant for module.
				var objPath = require.toUrl("dojo/tests/cache/object.html");
				t.is(expected, lang.trim(dojo.cache(new dojo._Url(objPath), {sanitize: true})));
